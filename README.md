...
* Præsenter **dele** af koden og beskriv den grundigt, hvor I kommer ind på løkker, forgreninger, variable og funktioner.
* Lav et rutediagram over spillet, beskrevet her [rutediagram](https://github.com/mpsteenstrup/InformatikRysensteen/blob/main/dokumenter/flowchart.md).
...
```
løkken kører så længe ```while```betingelsen er sand, dvs. så længe i er mindre end 4. I linjen ```i += 1``` bliver der lagt én til variablen i.
...
Vi sletter sidste element i listen med ```l.pop()``` og tilføjer i stedet 'informatik' med `l.push('informatik')`. Vi kan også ændre enkeltelementer fx. erstatte 'hej' med 'MP' ved at skrive ```l[0]='MP'```

## Gennemgang af programmet mmSimpleStart.js
Vi vil arbejde med programmet [mmSimpleStart.js](https://editor.p5js.org/mpsteenstrup/sketches/SKNtdTNPU). Spillet indeholder funktioner som er indbygget i p5JS biblioteket.
...
* ```setup()``` kører først.
* ```draw()``` er en løkke som kører igennem hele programmet.
...
### Billedeksempel
I filen [lister.js](programmer/lister.js) bruges en liste til at holde styr på flere billeder.
* Kør programmet og se hvad der sker.
* Undersøg hvordan billederne bliver tilføjet til listen.
* Undersøg hvordan de vises på skærmen.
* Hvad sker der, når du trykker på musseknappen?

### Lyd - det er sjovest med lyd
Filen [JSfiler/boing.js](JSfiler/boing.js) loader en mp3-fil og afspiller den hver gang musenklappen trykkes. boing.mp3 ligger i mappen "filer".
...
Reference kan findes her: [p5.sound](https://p5js.org/reference/#/libraries/p5.sound).

## Øvelse
* Hent filen og koden og prøv at få den til at køre. Hvis det ikke virker, så overvej hvordan stien til filen er lavet.
* Find lyde fx. på [https://freesound.org](https://freesound.org) og brug dem.
...
- Opret et issue eller lav et pull request på [GitHub-repositoriet](https://github.com/mpsteenstrup/spilLegOgSamarbejde).
- Du kan også skrive direkte med dine idéer eller fejl, du har fundet.
